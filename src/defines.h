...
#define BasicMethods(ClassType, Type)                                             \
  virtual void operator =(Type data) {TTypedProperty::operator=(data);}                \
  virtual void operator=(ClassType<Owner>& data) {TParameterProperty::operator=(data);}     \
  virtual void operator = (ClassType<Owner>* data) {TParameterProperty::operator=(data);}  \
  virtual string ToString() {return operator string();}                           \
  virtual void FromString(string val) {operator = (val);}                           \
  virtual void operator = (TBaseProperty<Owner>& data) { TParameterProperty::operator=((TParameterProperty<Type, Owner>*)(& data)); } \
